...
def build_etpc_maps(etpc_constraints):
    fore_ops_map = defaultdict(list)
    hind_ops_map = defaultdict(list)
    for constr in etpc_constraints:
        fore_job = constr.fore_job_orig_id_ref
        hind_job = constr.hind_job_orig_id_ref
        fore_op_idx = constr.fore_op_idx
        hind_op_idx = constr.hind_op_idx
        time_lapse = constr.time_lapse
        fore_ops_map[fore_job].append((fore_op_idx, hind_job, hind_op_idx, time_lapse))
        hind_ops_map[hind_job].append((hind_op_idx, fore_job, fore_op_idx, time_lapse))
    return fore_ops_map, hind_ops_map

def get_frWJETPC(job_sim_id: int, op_idx: int, job_weight: float, fore_ops_map) -> float:
    if any(fop_idx == op_idx for fop_idx, _, _, _ in fore_ops_map.get(job_sim_id, [])):
        return 2 * job_weight
    return job_weight

def get_hrTLETPC(job_sim_id: int, op_idx: int, hind_ops_map) -> float:
    lapses = [time_lapse for hop_idx, _, _, time_lapse in hind_ops_map.get(job_sim_id, []) if hop_idx == op_idx]
    return sum(lapses) if lapses else 0.0
...
def compute_priority(
    ...
    if rule_name == "GPISRule1":
        ...
        return max(rDDJ, rPTJ + hrTLETPC) / frWJETPC if frWJETPC != 0 else max(rDDJ, rPTJ + hrTLETPC)

    if rule_name == "GPISRule2":
        PTO = all_sim_jobs_ops[job_sim_id][op_idx_in_job][0][1] if all_sim_jobs_ops.get(job_sim_id) and len(all_sim_jobs_ops[job_sim_id]) > op_idx_in_job and all_sim_jobs_ops[job_sim_id][op_idx_in_job] else 0
        rDDJ = job_due_dates_map.get(job_sim_id, float('inf')) - current_simulation_time  # Relative Due Date of Job
        rPTJ = remaining_processing_time(all_sim_jobs_ops[job_sim_id], op_idx_in_job)
        frWJETPC = get_frWJETPC(job_sim_id, op_idx_in_job, job_weight, fore_ops_map)
...
def schedule_dynamic_no_parallel(
    ...
    fore_ops_map, hind_ops_map = build_etpc_maps(fjsp_instance.etpc_constraints)
